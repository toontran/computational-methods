...
def build_sigma_list():
    ...
    sigma_coarse = np.concatenate([
        ...
        2.0 * (2.0 ** np.arange(1, 6)),       # 2*2^(1:5) = 4, 8, 16, 32, 64
    ])
    ...
    sigma_refine = np.concatenate([
        np.arange(1.01, 1.04 + 0.005 / 2, 0.005),
        # np.array([
        #     1.04, 1.05, 1.06,
        #     1.10, 1.13, 1.15,
...
def build_U_hadamard_block():
    ...
    orthogonal block on R^{n-2}, following your MATLAB construction:

        Q1: 2x2 normalized Hadamard
        Q2: random (n-2)x(n-2) orthogonal (QR)
        U0 = blkdiag(Q1, Q2)
        move column 2 to the end (cols 1,3,4,...,n,2)
    """
    p = 2
    if p != 2:
...
def main():
    ...
    # Precompute A_template for every sigma per loop, since sigma changes
