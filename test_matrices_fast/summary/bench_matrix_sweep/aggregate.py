"""Aggregate per-matrix JSON summaries into a single table + synthesis."""
import json
import os
import glob
import sys
...
def main():
    files = sorted(glob.glob(os.path.join(HERE, "*_win64.json")))
    table = {}
    for fp in files:
        matrix = os.path.basename(fp).replace("_win64.json", "")
        table[matrix] = load_sliding(fp)

    # Add the already-done mixed-tail-sharp result
    msx = os.path.join(HERE, "..", "benchmark_online_vs_baselines_win64.json")
    if os.path.exists(msx):
        table["mixed-tail-sharp"] = load_sliding(msx)

    # Print per-metric table
    rows_md = []
    metrics = ["mean_exact_cos1", "mean_exact_cos2", "mean_relerr_sval",
               "final_exact_cos1", "final_exact_cos2", "final_tail_mass",
               "elapsed", "sec_per_step"]

    header = (
        f"{'matrix':<30} "
        + " ".join(f"{p[:8]:<9}" for p in POLICIES)
        + "  winner"
    )
    ...
    print(f"{'metric':<22} " + " ".join(f"{p[:8]:<9}" for p in POLICIES))
    for metric in metrics:
        higher = metric not in ("mean_relerr_sval", "final_tail_mass", "elapsed", "sec_per_step")
    ...
    # Per-policy elapsed summary
    print("\n=== Elapsed (s) summary across 17 matrices ===")
    print(f"{'policy':<24} {'min':<8} {'median':<8} {'mean':<8} {'max':<8} {'total':<8}")
    import statistics
    for p in POLICIES:
        vals = [rows[p]["elapsed"] for rows in table.values() if p in rows and rows[p].get("elapsed") is not None]
        if not vals:
            continue
        print(f"{p:<24} {min(vals):<8.2f} {statistics.median(vals):<8.2f} {sum(vals)/len(vals):<8.2f} {max(vals):<8.2f} {sum(vals):<8.1f}")
    ...
    for p in POLICIES:
        if p == "isvd":
            continue
        med = statistics.median([rows[p]["elapsed"] for rows in table.values() if p in rows])
        print(f"  isvd is {med/isvd_med:>5.1f}x faster than {p}")

    # Head-to-head: future_hmean_online vs each baseline
    print("\n=== future_hmean_online vs each baseline (wins/ties/losses across 17 matrices) ===")
    for p in ["combined", "hybrid", "isvd"]:
        for metric in metrics:
            higher = metric not in ("mean_relerr_sval", "final_tail_mass", "elapsed", "sec_per_step")
            w = l = t = 0
            for matrix, rows in table.items():
                if p not in rows or "future_hmean_online" not in rows:
                    continue
                vb = rows[p][metric]
                vo = rows["future_hmean_online"][metric]
                if vb != vb or vo != vo:
                    continue
                if abs(vo - vb) < 1e-6:
                    t += 1
                    ...
                    l += 1
            print(f"  vs {p:<10} {metric:<22} online W/T/L = {w}/{t}/{l}")
