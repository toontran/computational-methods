...
%
% For block w >= 2:
%   optimize the carried-state approximation
%     F(v)
%       = ((1-c)/2) * log(P(v))
%         + (c/4)   * log(Q(v)),
%   where
%     c    = 2 * log(rows_total / n) / log(rows_total),
%     P(v) = sum_j a_j^2 s_j^2 + ||A_w v||_2^2,
...
%% Basic parameters
n    = 1024;
r    = 5;               % target rank to keep in streaming
l    = 1;               % # true right singular vectors to track (must satisfy l <= r)
win  = 10;             % window / block size
mode = "iSVD";  % "iSVD", "FD", or "EntropyScore"
                ...
                case "EntropyScore"
                    ...
                    if start_row == 1
                        ...
                        if norm(e1_proj) > 1e-14
                            ...
                            score_vhat2 = entropy_score_fast( ...
                                    A_block, V_hat(:,1), end_row, n);
                            ...
                            if size(V_hat,2) >= 2 && norm(e2_proj) > 1e-14
                                score_e2_proj = entropy_score_fast( ...
                                    A_block, e2_proj / norm(e2_proj), end_row, n);
                                score_vhat2 = entropy_score_fast( ...
                                    A_block, V_hat(:,2), end_row, n);
                        ...
                        if norm(e1_proj) > 1e-14
                            score_e1_proj = entropy_score_fast( ...
                                A_block, e1_proj / norm(e1_proj), end_row, n, ...
                                state.prev_basis, state.prev_s2, state.prev_q);
                            score_vhat1 = entropy_score_fast( ...
                                A_block, V_hat(:,1), end_row, n, ...
                                state.prev_basis, state.prev_s2, state.prev_q);
            ...
            if is_initial_block
                ...
                [v_cand, logf_cand, s2_cand, H_cand] = ...
                    basic_projected_ascent_single_streaming( ...
                        A_block, prev_basis, prev_s2, prev_q, ...
...
function [v, logf, s2_total, H_approx] = basic_projected_ascent_single_streaming( ...
    A_block, V_old, s2_old, q_old, rows_total, n, v0, Q, maxit, tol)
    ...
    [logf, gradE, s2_total, H_approx] = entropy_streaming_logscore_grad( ...
        A_block, V_old, s2_old, q_old, v, rows_total, n);
        ...
        for ls_it = 1:20 %#ok<NASGU>
            ...
            if ~isempty(vt)
                [logf_trial, ~, ~, ~] = entropy_streaming_logscore_grad( ...
                    A_block, V_old, s2_old, q_old, vt, rows_total, n);
                rhs = logf_old + 1e-4 * alpha * real(g' * g);
                if logf_trial >= rhs
                    accepted = true;
                    v = vt;
...
function score = entropy_score_fast(M_or_Ablock, v, rows_total, n, varargin)
    if isempty(varargin)
        [logf, ~, ~, ~] = entropy_logscore_grad_rows(M_or_Ablock, v, rows_total, n);
        score = exp(logf);
        return;
    end

    if numel(varargin) ~= 3
        error('entropy_score_fast expects either 4 inputs (exact) or 7 inputs (streaming).');
    end
    ...
    P = sum((a.^2) .* s2_old) + kahan_sum(abs(y).^2);
    Q = sum((a.^4) .* q_old) + kahan_sum(abs(y).^4);

    if P <= 1e-28 || Q <= 1e-28 || any(~isfinite(y))
        score = 0;
        return;
    end

    Hhat = -log(Q / (P^2));
    log_score = 0.5 * log(P) - (log(rows_total / n) / (2 * log(rows_total))) * Hhat;
    score = exp(log_score);
end

function [logf, g, P, Happrox] = entropy_streaming_logscore_grad( ...
    A_block, V_old, s2_old, q_old, v, rows_total, n)
    ...
    % F(v) = ((1-c)/2) * log(P(v)) + (c/4) * log(Q(v))
    logf = 0.5 * (1 - c) * log(P) + 0.25 * c * log(Q);

    % dP/dv = 2 * V_old * (s2_old .* a) + 2 * A_block' * y
    gP = 2 * (V_old * (s2_old .* a)) + 2 * (A_block' * y);

    % dQ/dv = 4 * V_old * (q_old .* a.^3) + 4 * A_block' * (y.^3)
    gQ = 4 * (V_old * (q_old .* (a.^3))) + 4 * (A_block' * (y.^3));

    g = 0.5 * (1 - c) * (gP / P) + 0.25 * c * (gQ / Q);
